"""Algorithm 17: probe-free wide-dead-band variant of Algorithm 16.

Removes the staged pilot-probe machinery entirely and reallocates its compute
into direct coverage: the dead threshold widens from -3 to -4, so the
borderline neurons the probes used to rescue are simply kept sampled (bias
protection by inclusion). The on-side threshold stays at +3 because the
always-on fold is linear-exact and sampling those neurons buys nothing.
...
def _block_split_matmul(x, weights):
...
def _block_split_relu_matmul(x, weights):
    return fnp.maximum(_block_split_matmul(x, weights), 0.0)
    ...
    def _run_block(self, mlp: MLP, structure: dict, x, n_samples: int) -> list:
        ...
        for layer_idx, w in enumerate(mlp.weights):
            ...
            if layer_idx == 30 and len(on_idx) > 0 and prev_idx is not None:
                ...
                x_kink = _block_split_relu_matmul(x, w_kink)
            ...
            if layer_idx == 31 and x_before_fold is not None and len(on_indices[30]) > 0:
                ...
                pre_from_kink = _block_split_matmul(x, w_from_kink)
            ...
            if prev_idx is None:
                ...
                if _PACKED_ROWSPARSE and _PACKED_ROWSPARSE_START_LAYER <= layer_idx <= _PACKED_ROWSPARSE_STOP_LAYER:
                    if _BLOCK_SPLIT_ROWSPARSE:
                        x = _block_split_relu_matmul(x, w_active)
                    else:
