"""Algorithm 19 candidate: probe-free with layer-targeted dead band (-3 early, -4 late).

Probe machinery deleted (algorithm 18 lineage); dead threshold -3.0 for layers 0-29
and -4.0 for layers 30-31 (the layers feeding the scored final row), replacing the
probes' bias protection only where the score can see it. Original header follows.
...
_VAR_REF = 0.02143
_DEAD_THRESH = -3.0
_DEAD_THRESH_LATE = -4.0  # widened band only on the layers feeding the scored row
_DEAD_LATE_START = 30
_ON_THRESH = 3.0
...
class Estimator(BaseEstimator):
    ...
    def _initial_structure(self, mlp: MLP, width: int) -> dict:
        ...
        for layer_idx, w in enumerate(mlp.weights):
            ...
            layer_dead_thresh = _DEAD_THRESH_LATE if layer_idx >= _DEAD_LATE_START else _DEAD_THRESH
            dead_mask = alpha < layer_dead_thresh
