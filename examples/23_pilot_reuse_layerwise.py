"""Algorithm 23: pilot-reuse probes on the layer-wise fire-threshold surface.

Builds on Algorithm 21 / submission 315892 (per-layer _BLOCK_SPLIT_FIRE_THRESH
map) by adding Algorithm 19's pilot-reuse classification. Decisions and
predictions were verified identical to the staged probes on the algo17
surface; flops -0.25% deterministic. Rejected 2026-07-11 on local residual
(+8.9%, the per-layer activation concat), resurrected after the 315892 grade
showed the grader prices memory-traffic wall time at ~zero - the concat
copies are the same free-to-the-grader class as the packed gathers.

Original Algorithm 19 header follows.
