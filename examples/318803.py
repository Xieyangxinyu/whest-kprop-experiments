"""Fixed 61,440-sample estimator: antithetic pilots, 2-level Strassen, cold-column slicing.
...
guarded 2-level recursive Strassen decomposition. During the pilot block each
layer records per-column fire rates; the continuation block then orders columns
coldest-first (via the producing weight slice), partitions rows by cold-column
support with a scatter, and routes the hot and cold blocks of each contraction
separately through contiguous slices. Weights are cast to float32 once per
layer. The final row is sampled accurately, while intermediate returned rows
are cheap analytical fillers.
"""
...
_COLD_REORDER = "put"   # 'put' = 1/elem scatter; 'take' = 4/elem gather fallback
    ...
    def _run_block(self, mlp: MLP, structure: dict, x, n_samples: int, refine: bool, cold_plan=None) -> tuple[list, float, dict]:
        ...
        for layer_idx, w in enumerate(mlp.weights):
            ...
            if prev_idx is None:
                ...
                kc = 0 if (refine or not cold_plan) else cold_plan.get(layer_idx, 0)
                if kc:
                    x = _cold_slice_relu_matmul(x, w_active, kc)
                else:
