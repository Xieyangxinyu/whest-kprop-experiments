...
moment propagation runs in float32 (stats outputs cast back down), and
constructed index vectors use int32. The final row is sampled accurately,
while intermediate returned rows are analytical fillers.
"""
...
def _cold_slice_relu_matmul(x, w_active, kc):
    ...
    n = x.shape[0]
    cold_nnz = fnp.sum(x[:, :kc] > fnp.float32(0.0), axis=1, dtype=fnp.float32)
...
def _cold_slice_relu_matmul2(x, w_active, kc, kc2):
    ...
    n = x.shape[0]
    hu = fnp.sum(x[:, :kc2] > fnp.float32(0.0), axis=1, dtype=fnp.float32) > fnp.float32(0.0)
    hw = fnp.sum(x[:, kc2:kc] > fnp.float32(0.0), axis=1, dtype=fnp.float32) > fnp.float32(0.0)
    ...
    def _run_block(self, mlp: MLP, structure: dict, x, n_samples: int, refine: bool, cold_plan=None) -> tuple[list, float, dict]:
        ...
        for layer_idx, w in enumerate(mlp.weights):
            ...
            if (
                ...
                fire_stats[layer_idx] = fnp.sum(x > fnp.float32(0.0), axis=0, dtype=fnp.float32)
...
def _plan_cold_slices(structure, n_pilot: int):
    ...
    for j in range(_COLD_START_LAYER, _COLD_STOP_LAYER + 1):
        ...
        if acts == 0 or out_j == 0 or len(fire_cnt) != acts:
            continue
        k = int(fnp.sum(fire_cnt < thresh_cnt, dtype=fnp.float32))
        k = min(k, _COLD_MAX_K, acts - _COLD_MIN_HOT_DIM)
        if k < _COLD_MIN_K:
            continue
        k2 = int(fnp.sum(fire_cnt < thresh2_cnt, dtype=fnp.float32))
