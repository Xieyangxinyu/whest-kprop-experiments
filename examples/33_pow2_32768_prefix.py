"""Algorithm 33: power-of-2 prefix truncation (N = 32,768 = 2^15 for every net).

Identical surface and artifact to Algorithm 31 (submission 317197) except
_TOTAL_SAMPLES = 32,768: 16,384 = 2^14 Sobol half-samples x 2 (antithetic),
a PREFIX of the shipped leaderboard-validated 30,720-half realization (same
scramble, no artifact re-roll). Clean leaderboard instrument for the
power-of-2 (t,m,d)-net balance hypothesis, isolated from realization luck
-- unlike 317412, which confounded it with a fresh seed-1005 artifact and
regressed +20.2% adjusted. Offline probe (2026-07-19) predicts no balance
dip; the adjusted(N) bowl and the 317172-vs-shorter-prefix pair predict
worse adjusted (raw up ~2x reducible, multiplier down ~47%-class). Base
10,240 + one 22,528-sample continuation, weighted merge = plain prefix mean.
...
_BASE_SAMPLES = 10240
_TOTAL_SAMPLES = 32768  # 2^15: 16384 (=2^14) Sobol half-samples x 2 (antithetic)
_DEAD_THRESH = -3.0
_ON_THRESH = 3.0
