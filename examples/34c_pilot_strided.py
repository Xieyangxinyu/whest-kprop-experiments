# DEAD VARIANT (P3-strided, 2026-07-19, never submitted): even-spread (every-10th-row) pilot rows at unchanged 512/2,048 counts.
# Paired seed-42 10-MLP result vs Algorithm 31: raw +0.32% (outlier +5.2%) at equal flops - Sobol-prefix balance is load-bearing.
...
def _sample_alpha(x, weights, rows: int):
    total = x.shape[0]
    if rows < total:
        idx = (fnp.arange(rows) * total) // rows
        x = fnp.take(x, idx, axis=0)
    pre = x[:rows, :] @ weights
