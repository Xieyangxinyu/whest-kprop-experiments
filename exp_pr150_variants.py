"""Parameterized Algorithm 34 surface for PR #150 repricing experiments.

Ported from the verified submission 317421 bytes
(`submissions/algo34-antithetic-pilot/estimator.py`, sha 0442dc32-era) with
every experiment knob lifted into module config. With `reset_config()` (all
defaults) the execution path is bit-identical to Algorithm 34.

Experiment switches (see `CONFIG_DEFAULTS`):

- `_CAST_WEIGHTS_F32`   dtype hygiene: cast mlp.weights float64 -> float32 once
                        per layer before ANY op touches them. Under flopscope
                        0.9.1 four-factor billing, f64@f64 matmul bills 5x the
                        f32 cost and mixed f32@f64 bills 3x, so the default
                        False (= Algorithm 34 behavior) is the audit baseline
                        and True is the fix.
- `_PACKED_ROWSPARSE` / `_BLOCK_SPLIT_*` / `_PACKED_ROWSPARSE_MAX_K_*`
                        idea 1 / idea 5: the gather-packed row-sparse machinery
                        and its routing thresholds (PR150 repriced `take` to
                        4/elem, so old crossovers are wrong).
- `_LAYER1_FULL_PACK`   idea 5: False routes layer 1 through fire-rate
                        block-split like other layers instead of full packing.
- `_PILOT_MODE`         idea 3: 'staged' = 5% primary + 20% recheck (baseline),
                        'single20' = skip the 5% stage, classify every analytic
                        probe candidate with the 20% sampled alpha directly.
- `_FOLD_ENABLED`       idea 2: False disables the layer-30/31 on-neuron fold;
                        on-neurons stay in the sampled path through the end.
- `_IDENTITY_ON_START_LAYER`
                        idea 2: layers l with start <= l <= 29 thread on-neuron
                        columns as identity (no ReLU) — accuracy probe for
                        early linearization (99 = off).
- `_DEAD_REPORT_SCALE`  idea 4: final-row dead correction is scaled by this
                        (1.0 = analytic baseline, 0.0 = report zeros).
- `CAPTURE`             idea 4: set `CAPTURE['on'] = True` and after predict()
                        read CAPTURE['mc_final' / 'dead_corr_final' / class
                        index arrays] to sweep dead-report variants post hoc
                        without re-running.
"""
...
# --- Algorithm 34 baseline constants (do not edit; override via configure) ---
_BASE_SAMPLES = 10240
...
# --- experiment switches (defaults = Algorithm 34 behavior) ---
_CAST_WEIGHTS_F32 = False
_LAYER1_FULL_PACK = True
_PILOT_MODE = "staged"          # 'staged' | 'single20'
_FOLD_ENABLED = True
_IDENTITY_ON_START_LAYER = 99   # 99 = off; e.g. 25 threads on-cols as identity on layers 25..29
_DEAD_REPORT_SCALE = 1.0
...
CONFIG_DEFAULTS = {
    ...
    if k.startswith("_") and not k.startswith("__") and k == k.upper()
}


def configure(**overrides):
    """Set module config; unknown keys raise. Returns the applied dict."""
    for key, value in overrides.items():
        if key not in CONFIG_DEFAULTS:
            raise KeyError(f"unknown config key {key!r}")
        globals()[key] = value
    return {k: globals()[k] for k in overrides}
...
def current_config():
    return {k: globals()[k] for k in CONFIG_DEFAULTS}


def config_tag(overrides: dict) -> str:
    """Stable short tag for cache filenames."""
    items = sorted((k, repr(v)) for k, v in overrides.items())
    import hashlib

    h = hashlib.md5(repr(items).encode()).hexdigest()[:10]
    return h
...
def _staged_threshold_split(source_idx, x, weights, n_samples: int, threshold: float):
    ...
    if _PILOT_MODE == "single20":
        alpha = _sample_alpha(x, weights, recheck_rows)
        above_mask = alpha > threshold_value
...
def _block_split_matmul_2blk(x2, weights, layer_idx: int):
    ...
    if layer_idx == _PACKED_ROWSPARSE_START_LAYER and _LAYER1_FULL_PACK:
        return (
            _packed_matmul(x2[0], weights, masks2[0]),
            _packed_matmul(x2[1], weights, masks2[1]),
        )
...
def _selective_relu_2blk(pre2, n_kink_cols: int):
    """ReLU the first n_kink_cols columns, thread the rest (on-cols) as identity."""
    top, bottom = pre2
    return (
        fnp.concatenate([fnp.maximum(top[:, :n_kink_cols], 0.0), top[:, n_kink_cols:]], axis=1),
        fnp.concatenate([fnp.maximum(bottom[:, :n_kink_cols], 0.0), bottom[:, n_kink_cols:]], axis=1),
    )
...
class Estimator(BaseEstimator):
    ...
    def _initial_structure(self, weights_list, depth: int, width: int) -> dict:
        active_indices = []
        kink_indices = []
        on_indices = []
        dead_indices = []
        ...
        for layer_idx, w in enumerate(weights_list):
            ...
            if len(dead_idx) > 0 and (_MATERIALIZE_INTERMEDIATE_ROWS or layer_idx == depth - 1):
    ...
    def _run_block(self, weights_list, depth: int, width: int, structure: dict, x, n_samples: int, refine: bool):
        ...
        for layer_idx, w in enumerate(weights_list):
            ...
            if _FOLD_ENABLED and layer_idx == 30 and len(on_idx) > 0 and prev_idx is not None:
                x_before_fold = x
                ...
                if _PACKED_ROWSPARSE and _BLOCK_SPLIT_ROWSPARSE:
                    x_kink = _block_split_relu_matmul_2blk(x, w_kink, layer_idx)
                else:
                    dt, db = _dense_matmul_2blk(x, w_kink)
                    x_kink = (fnp.maximum(dt, 0.0), fnp.maximum(db, 0.0))
            ...
            if _FOLD_ENABLED and layer_idx == 31 and x_before_fold is not None and len(on_indices[30]) > 0:
                ...
                if _PACKED_ROWSPARSE and _BLOCK_SPLIT_ROWSPARSE:
                    ...
                    pre_from_kink = _dense_matmul_2blk(x, w_from_kink)

                w_fold_layer = weights_list[30]
            ...
            identity_layer = (
                ...
                and _IDENTITY_ON_START_LAYER <= layer_idx <= 29
                and len(on_idx) > 0
                and len(kink_idx) > 0
            )
            ...
            if prev_idx is None:
                ...
                if identity_layer:
                    # column order [kinks, ons] so the identity block is a suffix
                    idx = fnp.concatenate([kink_idx, on_idx])
                    active_indices[layer_idx] = idx
                ...
                if identity_layer:
                    if use_packed and _BLOCK_SPLIT_ROWSPARSE:
                        pre2 = _block_split_matmul_2blk(x, w_active, layer_idx)
                    elif use_packed:
                        pre2 = (_packed_matmul(x[0], w_active), _packed_matmul(x[1], w_active))
                    else:
                        pre2 = _dense_matmul_2blk(x, w_active)
                    x = _selective_relu_2blk(pre2, len(kink_idx))
                elif use_packed:
        ...
        if _MATERIALIZE_INTERMEDIATE_ROWS:
            ...
            if _DEAD_REPORT_SCALE == 1.0:
                final_row = mc_rows[-1] + dead_corrections[-1]
            elif _DEAD_REPORT_SCALE == 0.0:
                final_row = mc_rows[-1]
            else:
                final_row = mc_rows[-1] + dead_corrections[-1] * fnp.float32(_DEAD_REPORT_SCALE)
            rows = list(analytical_rows[:-1]) + [final_row]
    ...
    def predict(self, mlp: MLP, budget: int) -> fnp.ndarray:
        ...
        if _CAST_WEIGHTS_F32:
            weights_list = [fnp.asarray(w).astype(fnp.float32) for w in mlp.weights]
        else:
            weights_list = list(mlp.weights)
        structure = self._initial_structure(weights_list, depth, width)
        base_x = self._sample_block(0, _BASE_SAMPLES // 2, width)
        base_rows, refined_structure = self._run_block(
            weights_list, depth, width, structure, base_x, _BASE_SAMPLES, refine=True
        )
        ...
        if extra_samples > 0:
            ...
            extra_rows = self._run_block(
                weights_list, depth, width, refined_structure, extra_x, extra_samples, refine=False
            )[0]
            combined_rows = [
        ...
        if CAPTURE.get("on"):
            corr = refined_structure["dead_corrections"][-1]
            final = combined_rows[-1]
            CAPTURE["dead_corr_final"] = np.asarray(corr, dtype=np.float64)
            CAPTURE["final_row"] = np.asarray(final, dtype=np.float64)
            CAPTURE["kink_idx_final"] = np.asarray(refined_structure["kink_indices"][-1])
            CAPTURE["on_idx_final"] = np.asarray(refined_structure["on_indices"][-1])
            CAPTURE["dead_idx_final"] = np.asarray(refined_structure["dead_indices"][-1])
