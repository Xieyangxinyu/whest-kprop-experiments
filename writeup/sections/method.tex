...
\subsubsection{Algorithmic Use of the Regime Split}
...
\subsection{Dense/Sparse Blocks in Sampled Activations}
...
The dead/kink/on split reduces compute primarily by removing the computation for dead neurons, but the surviving kink neurons still exhibit substantial sparsity that can be exploited. To understand this structure, we examined the sampled post-ReLU activation matrix for each layer, reordering columns by firing rate—the fraction of Monte Carlo samples for which each neuron is active (i.e., has a positive post-ReLU activation). This reveals a characteristic structure (\Cref{fig:activation-design-matrix}). Dead columns are blank, high-fire columns are dense across sample rows, and the middle columns form a row-dependent sparse block. These observations motivate a simple implementation rule: multiply the dense block using conventional dense matrix multiplication, and compute the remaining sparse block using exact row-sparse matrix multiplication, which skips zero entries without approximation.

\subsubsection{Sample Propagation}
...
We use a global column-firing threshold of 0.5 to partition dense and sparse blocks. Large dense matrix products are further accelerated with a guarded one-level Strassen multiplication \citep{strassen1969gaussian}. 
